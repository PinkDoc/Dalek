...
public:
  inline HttpConnection(EventLoop &looper, TimerWheel &wheel, int fd)
      : fd_(fd), channel_(looper, fd), looper_(&looper), timer_wheel_(&wheel),
        request_(), parser_(request_), handle_state_(READ_NOT_READY),
        file_fd_(-1)
...
public:
  ...
  inline HttpServer(EventLoop &looper, TimerWheel &wheel)
      : looper_(&looper), timer_wheel_(&wheel),
        fd_(::socket(AF_INET, SOCK_STREAM, 0)), channel_(looper, fd_),
        address_(8000, false), socket_(fd_) {
    assert(fd_ > 0);
